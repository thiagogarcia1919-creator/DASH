...
def same_oee_record(a, b):
    """Compara OEE por SEQ+Linha; usa Data+Turno como compatibilidade."""
    seq_a, seq_b = id_text(a.get("seq")), id_text(b.get("seq"))
    linha_a = str(a.get("linha") or STATE.get("linha") or "LINHA 04").strip().upper()
    linha_b = str(b.get("linha") or STATE.get("linha") or "LINHA 04").strip().upper()
    if seq_a and seq_b:
        return seq_a == seq_b and linha_a == linha_b
    return (
        linha_a == linha_b
...
def same_lote_record(a, b):
    """Compara programação por Item+Linha; usa OP/Lote como compatibilidade."""
    ...
    if item_a and item_b:
        return item_a == item_b and linha_a == linha_b
    op_a, op_b = str(a.get("op") or "").strip(), str(b.get("op") or "").strip()
    if op_a and op_b:
        return op_a == op_b and linha_a == linha_b
    lote_a, lote_b = str(a.get("lote") or "").strip(), str(b.get("lote") or "").strip()
    return bool(lote_a and lote_b and lote_a == lote_b and linha_a == linha_b)
